...
public class KechengxinxiController {
    ...
    public R page(@RequestParam Map<String, Object> params,KechengxinxiEntity kechengxinxi,
		...
		if(tableName.equals("jiaolian")) {
			kechengxinxi.setJiaolianzhanghao((String)request.getSession().getAttribute("username"));
		}
		if(tableName.equals("xueyuan")) {
			kechengxinxi.setXueyuanzhanghao((String)request.getSession().getAttribute("username"));
		}
	...
	@IgnoreAuth
    @RequestMapping("/yuyue/{id}/{xueyuanid}")
    public R yuyue(@PathVariable("id") Long id, @PathVariable("xueyuanid") Long xueyuanid){
		KechengxinxiEntity kechengxinxi = kechengxinxiService.selectById(id);
		XueyuanEntity xueyuan = xueyuanService.selectById(xueyuanid);
		List<KechengyuyueView> kechengxinxiViews = kechengyuyueService.selectListView(new EntityWrapper<KechengyuyueEntity>().eq("kechengbianhao",kechengxinxi.getKechengbianhao()).eq("xueyuanzhanghao",xueyuan.getXueyuanzhanghao()));
		if(kechengxinxiViews.size()>0){
			return R.error("该课程已预约，不可重复预约！");
		}else{
			KechengyuyueEntity kechengyuyue = new KechengyuyueEntity<>();
			kechengyuyue.setKechengbianhao(kechengxinxi.getKechengbianhao());
			kechengyuyue.setJiaolianzhanghao(kechengxinxi.getJiaolianzhanghao());
			kechengyuyue.setJiaolianxingming(kechengxinxi.getJiaolianxingming());
			kechengyuyue.setAddtime(new Date());
			kechengyuyue.setKechengjieshao(kechengxinxi.getKechengjieshao());
			kechengyuyue.setKechengshipin(kechengxinxi.getKechengshipin());
			kechengyuyue.setKechengmingcheng(kechengxinxi.getKechengmingcheng());
			kechengyuyue.setKechengtupian(kechengxinxi.getKechengtupian());
			kechengyuyue.setShangkeshijian(kechengxinxi.getShangkeshijian());
			kechengyuyue.setShangkedidian(kechengxinxi.getShangkedidian());
			kechengyuyue.setXueyuanzhanghao(xueyuan.getXueyuanzhanghao());
			kechengyuyue.setXueyuanxingming(xueyuan.getXueyuanxingming());
